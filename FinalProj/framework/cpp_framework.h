...
namespace CCP {
	...
	class System {
	public:
		inline static tick_t read_cpu_ticks() {return RDTICK();}

		inline static tick_t currentTimeMillis() {
			timeb tp;
			ftime(&tp);
			return 1000*(tp.time) + tp.millitm;
		}

		inline static void exit(const int exit_code) {
			::exit(exit_code);
		}
	};
	...
	class Math {
	public:
		inline static double ceil(double a) {return ::ceil(a);}
		inline static double floor(double a) {return ::floor(a);}

		inline static int Max(final int a, final int b) {if(a >= b) return a; else return b;}
		inline static int Min(final int a, final int b) {if(a <= b) return a; else return b;}
	};
	...
	public:
		...
		// inline static void* byte_aligned_malloc(const size_t size) {return ALIGNED_MALLOC(size, (void**)CACHELINE_SIZE);}
		// inline static void* byte_aligned_malloc(const size_t size, const size_t alignment) {return ALIGNED_MALLOC(size,alignment);}
		inline static void  byte_aligned_free(void* mem)	{ALIGNED_FREE(mem);}

		inline static void  read_write_barrier()	{ MB();	}
		inline static void  write_barrier()		{ WMB(); }
		inline static void  read_barrier()			{ RMB(); }

		inline static _u32  compare_and_set(_u32 volatile* _a, _u32 _o, _u32 _n)		{return CAS32(_a,_o,_n);}
		inline static void* compare_and_set(void* volatile * _a, void* _o, void* _n)	{return (void*)CASPO(_a,_o,_n);}
		inline static _u64  compare_and_set(_u64 volatile* _a, _u64 _o, _u64 _n)		{return CAS64(_a,_o,_n);}

		inline static _u32  exchange_and_set(_u32 volatile *   _a, _u32  _n)	{return SWAP32(_a,_n);}
		inline static void* exchange_and_set(void * volatile * _a, void* _n)	{return SWAPPO(_a,_n);}
	};
	...
	class VolatileType {
		V volatile _value;
	public:
		inline explicit VolatileType() { }
		inline explicit VolatileType(const V& x) {
			_value = x;
		...
		inline V getNotSafe() const {
			return _value;
		}

		inline void set(const V& x) {
			_value = x;
			Memory::write_barrier();
		}
		inline void setNotSafe(const V& x) {
			_value = x;
		}

		//--------------
		inline operator V() const {  //convention
			return get();
		}
		inline V operator->() const {
			return get();
		}
		inline V volatile * operator&() {
			Memory::read_barrier();
			return &_value;
		}

		//--------------
		inline VolatileType<V>& operator=(const V x) { 
			_value = x;
			Memory::write_barrier();
			return *this;
		}
		//--------------
		inline bool operator== (const V& right) const {
			Memory::read_barrier();
			return _value == right;
		}
		inline bool operator!= (const V& right) const {
		...
		inline bool operator== (const VolatileType<V>& right) const {
			Memory::read_barrier();
		...
		//--------------
		inline VolatileType<V>& operator++ () { //prefix ++
			++_value;
			Memory::write_barrier();
			return *this;
		}
		inline V operator++ (int) { //postfix ++
			const V tmp(_value);
			++_value;
			Memory::write_barrier();
			return tmp;
	...
	public:
		...
		inline long awaitNanos(long nanosTimeout) {
			pthread_mutex_lock(&_lock);
	...
	public:
		...
		inline void setNotSafe(const int newValue) {
			_value.setNotSafe(newValue);
		}

		inline bool compareAndSet(const int expect, const int update) {
			return (expect == (const int)CAS32(&_value, expect, update));
		}  
		inline int getAndIncrement() {
			do {
				const int value = _value;
				if (value == (const int)CAS32(&_value, value, value + 1))  {
					return value;		
				}
			} while(true);
		}
		...
		inline int getAndSet(const _u32 newValue) {
			return Memory::exchange_and_set((_u32 volatile*)&_value, newValue);
		}
		inline int getAndAdd(const int delta) {
			do {
				const int value = _value;
				if (value == (const int)CAS32(&_value, value, value+delta)) {
					return value;		
				}
			} while(true);
		} 
		inline int addAndGet(const int delta) {
			do {
				const int value = _value;
				if (value == (const int)CAS32(&_value, value, value+delta)) {
					return value+delta;
		...
		inline _u64 longValue()  {
			return (_u64)_value;
		}
		inline int get() {
			return _value;
		} 
		inline int getNotSafe() {
	...
	public:
		...
		inline void set(const _u64 newValue) {
			_value = newValue;
		}
		inline bool compareAndSet(const _u64 expect, const _u64 update) {
			return (expect == CAS64(&_value, expect, update));
		}  
		inline _u64 getAndIncrement() {
			do {
		...
		inline _u64 getAndSet(const _u64 newValue) {
			do {
				_u64 value = _value;
		...
		inline _u64 getAndAdd(const _u64 delta) {
			do {
				const _u64 value = _value;
		...
		inline _u64 addAndGet(const _u64 delta) {
			do {
				const _u64 value = _value;
				if (value == CAS64(&_value, value, value+delta)) {
					return value+delta;
	...
	public:
		...
		inline void set(const bool newValue) {
			if(newValue)
				_value = TRUE;
			else
				_value = FALSE;
		...
		inline bool compareAndSet(const bool expect, const bool update) {
			int i_expect, i_update;
			if(expect) i_expect=TRUE; else i_expect=FALSE; 
			if(update) i_update=TRUE; else i_update=FALSE; 
			return (i_expect == (int)CAS32(&_value, i_expect, i_update));
		}  

		inline bool getAndSet(const bool newValue) {
			int i_newValue;
			if(newValue) i_newValue=TRUE; else i_newValue=FALSE; 

			do {
				int value = _value;
				if (value == (int)CAS32(&_value, value, i_newValue)) {
					return (TRUE == value);		
				}
			} while(true);
		}
	...
	public:
		...
		inline void set(V* newValue) {
			_value.set(newValue);
		}

		inline V* getAndSet(const V* newValue) {
			return (V*) Memory::exchange_and_set((void * volatile *)&_value, (void*)newValue);
		}

		inline bool compareAndSet(const V* expect, const V* update) {
		...
		inline V* getReference() const {
			return _value.get();
		}
		inline V* getRefNotSafe() const {
			return _value.getNotSafe();
		}
	};
	...
	public:
		...
		inline operator V*() {  //convention
			return getReference();
		}
		inline V* operator->() {
			return getReference();
		}
		inline bool attemptMark(V* const expectedReference, const bool newMark)  {
			V* exp_value;
			V* new_value;
		...
		inline bool compareAndSet(V* const expectedReference, V* const newReference, const bool expectedMark, const bool newMark) {
			V* expexted_value;
			V* new_value;
			if(expectedMark) {
		...
		inline _u32 getStamp() {
			if(0 == is_marked_ref(_value.get()))
				return 0;
			else return 1;
		}

		inline void set(V* newReference, bool newMark) {
			if(newMark) {
		...
		inline bool operator==(const AtomicMarkableReference& AMR)	{
		...
		inline void operator=(const AtomicMarkableReference& AMR) {/*error*/
			bool volatile otherMark;
			V* const otherPtr = AMR.get(&otherMark);
			set(otherPtr, otherMark);
		}

		inline bool isEqual(V* const in_ref, const _u32 in_stamp) const {
			bool	thisMark;
		...
		public:
			...
			inline V* get(_u32 volatile * stampHolder) const {
				_u64 save_value = _value.get();
				*stampHolder = ((_u32) (save_value & 0xFFFFFFFFUL));
				return (V*) ((ptr_t)(save_value >> 32));
			}

			inline V* getNotSafe(_u32 volatile * stampHolder) const {
			...
			inline bool compareAndSet(V* const expectedReference, V* const newReference, const _u32 expectedStamp, const _u32 newStamp) {
			...
			inline void set(V* const newReference, const _u32 newStamp) {
				_value = ((((_u64)newReference) << 32) | ((_u64)newStamp));
			}
			inline void setNotSafe(V* const newReference, const _u32 newStamp) {
				_value.setNotSafe( ((((_u64)newReference) << 32) | ((_u64)newStamp)) );
			}

			inline bool attemptStamp(V* const expectedReference, const _u32 newStamp) {
				_u64 save_value = _value.get();
			...
			inline bool operator==(const AtomicStampedReference& ASR)	{
			...
			inline void operator=(const AtomicStampedReference& ASR) {/*error*/
				_u32 volatile otherMark;
				V* const otherPtr = ASR.get(&otherMark);
	...
	public://int functionality
		...
		inline int compareTo(const Integer& anotherInteger) {
			int thisVal = _value;
	...
	public://general int functionality
		...
		// Bit twiddling
		inline static std::string toString(int i, int radix) {
			char tmp_buf[256*1];
			Itoa (i, tmp_buf,radix);
			return std::string(tmp_buf);
		}

		inline static std::string toHexString(const int i) {
			std::stringstream lStream; 
		...
		inline static std::string toOctalString(const int i) {
			std::stringstream lStream; 
		...
		inline static std::string toBinaryString(const int i) {
			char tmp_buf[256*1];
			Itoa (i,tmp_buf,2);
			return std::string(tmp_buf);
		}

		inline static std::string toString(const int i) {
			std::stringstream lStream;
			lStream << i;
			return lStream.str();
		}
		...
		// Requires positive x
		inline static int parseInt(const char* s, const int radix) {
			return strtol(s,null,radix);
		}

		inline static int parseInt(const char* s) {
			return parseInt(s,10);
		}

		inline static int highestOneBit(int i) {
			// HD, Figure 3-1
			i |= (i >>  1);
			i |= (i >>  2);
			i |= (i >>  4);
		...
		inline static int lowestOneBit(const int i) {
			// HD, Section 2-1
			return i & -i;
		}

		inline static int numberOfLeadingZeros(int i) {
			// HD, Figure 5-6
			if (i == 0)
				return 32;
			int n = 1;
		...
		inline static int numberOfTrailingZeros(int i) {
			// HD, Figure 5-14
			int y;
			if (i == 0) return 32;
			int n = 31;
		...
		inline static int rotateLeft(const int i, const int distance) {
			return (i << distance) | (i >> -distance);
		}

		inline static int rotateRight(const int i, const int distance) {
			return (i >> distance) | (i << -distance);
		}

		inline static int reverse(int i) {
			// HD, Figure 7-1
		...
		inline static int signum(const int i) {
			// HD, Section 2-7
			return (i >> 31) | (-i >> 31);
		}

		inline static int reverseBytes(const int i) {
			return ((i >> 24)           )  |
				((i >>   8) &   0xFF00)  |
				((i <<   8) & 0xFF0000)  |
				((i << 24));
		...
		inline static _u32 nearestPowerOfTwo(const _u32 i)	{
			_u32 rc( 1 );
			while (rc < i) {
				rc <<= 1;
			}
		...
		inline static _u32 log2(const _u32 i) {
			_u32 mask = 1;
			_u32 count=0;
			while(mask < i) {
				mask <<= 1;
		...
		inline static _u32 bitCount(_u32 i) {
			return bit_count(i);
		}

		inline static _u32 lsbBitIndx(_u32 x) {
			return first_lsb_bit_indx(x);
		}

		inline static _u32 msbBitIndx(_u32 x) {
			return first_msb_bit_indx(x);
		}

	private:
		inline static char* Itoa(int value, char* str, int radix) {
	...
	public://NOT THREAD SAFE, EACH THREAD SHOULD USE DIFFRENT OBJECT !!
		inline double nextUniform(void) {
			double uni;
		...
		inline double nextGaussian(double mean, double stddev) {
			double  q,u2,v,x,y;
		...
		inline unsigned int nextInt(int upper) {
			return nextInt(0, upper);
		}

		inline _u64 nextLong(_u64 lower,_u64 upper) {
			//Return random integer within a range, lower -> upper INCLUSIVE
		...
		inline _u64 nextLong( _u64 upper) {
			return nextLong(0, upper);
		}

		inline bool nextBoolean() {return (0 != (nextInt(1024) & 1));}
		inline double nextDouble(double lower,double upper) {
			//Return random float within a range, lower -> upper
			return((upper - lower) * nextUniform() + lower);
		}
		inline double nextDouble(double upper) {
			return nextDouble(0, upper);
		}

	public://THREAD SAFE !!
		inline static _u64 getSeed() {
			//get random the seed
			return (time(0)*System::read_cpu_ticks()) ^ (clock() * System::read_cpu_ticks());
		}
		inline static _u64 getRandom(_u64 seed) {
			if(0==seed)
				seed = getSeed();
			seed = seed * 196314165 + 907633515;
			return seed;
		}
		inline static int getRandom(_u64 volatile& seed, int inValue) {
		...
		inline static int getRandomScatter(_u64 seed, int inValue) {
	...
	class SnapshotCounter {
		...
		//helper functions ----------------------------
		inline static int get_size(const _u64 counterAndValue) {
			return (int)(counterAndValue & 0xFFFFFFFFL);
		}
		inline static int get_seq(const _u64 counterAndValue) {
			return (int)(counterAndValue >> 32);
		}
		inline static _u64 build_seq_value(const int seq, const int value) {
		...
		struct ProcessData {
			...
			inline int recent_seq() const  {
				return get_seq(_recent_seq_value);
			}
			inline int recent_seq_int() const  {
				return get_seq(_recent_seq_value);
			}
			inline int prev_seq() const  {
				return get_seq(_prev_seq_value);
			}
			inline int recent_value() const {
				return get_size(_recent_seq_value);
			}
			inline int prev_value() const {
				return get_size(_prev_seq_value);
			}

			inline void update(const int counter, const int value) {
				_recent_seq_value = build_seq_value(counter, value);
			}
			inline void update_prev(const int counter, const int value) {
				_prev_seq_value = build_seq_value(counter, value);
			}
			inline void set_prev_to_curr() {
	...
	public:
		...
		inline void update(const int iProcess, const int new_value) {
		...
		inline void inc(const int iProcess) {
			ProcessData& processData = _gMem[iProcess];			
			const int update_seq = _gSeq.get();
			if (update_seq != processData.recent_seq())
				processData.set_prev_to_curr();
		...
		inline void dec(const int iProcess) {
			ProcessData& processData = _gMem[iProcess];			
			const int update_seq = _gSeq.get();
			if (update_seq != processData.recent_seq())
				processData.set_prev_to_curr();
		...
		inline void add(const int iProcess, const int x) {
			ProcessData& processData = _gMem[iProcess];			
			const int update_seq = _gSeq.get();
			if (update_seq != processData.recent_seq())
				processData.set_prev_to_curr();
		...
		inline int valueRequest(const int iProcess) {
	...
	/////////////////////////////////////////////////////////////////////////////
		...
		inline _u32 ptr_hash_func(ptr_t key) {
			key = ~key + (key << 15);
			key = key ^ (key >> 12);
			key = key + (key << 2);
			key = key ^ (key >> 4);
